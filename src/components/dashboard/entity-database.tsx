    ...
    <Card className="flex-1 flex flex-col border-primary/50 bg-primary/10">
      <CardHeader className="py-3 px-4">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-lg text-accent">ENTITY DATABASE</CardTitle>
        </div>
      </CardHeader>
        ...
        <Accordion type="single" collapsible className="w-full">
          {entities.map((entity) => (
            <AccordionItem key={entity.id} value={entity.id} className="border-accent/10">
              <AccordionTrigger className="hover:no-underline hover:bg-accent/10 rounded-md px-2 py-2 text-base">
                <div className="flex items-center gap-3">
                  <TypeIcon type={entity.type} />
                  <span>{entity.name}</span>
                </div>
              </AccordionTrigger>
              <AccordionContent className="p-4 space-y-3 text-muted-foreground bg-background/50 rounded-b-md">
                <p>{entity.description}</p>
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 flex-wrap">
                      <Tag className="h-4 w-4 text-accent/50"/>
                      {entity.tags.map((tag) => (
                        <Badge key={tag} variant="secondary" className="bg-primary/80 text-foreground/80">
                          {tag}
                        </Badge>
                      ))}
                    </div>
